...
public class SecureFilter implements Filter, SecurityConstants
{
	...
	private boolean checkActionSubmitInGet(String queryString, String actionSubmit)
	{
		int isActionOrSubmitForm = queryString.indexOf(actionSubmit); 
		if(isActionOrSubmitForm != -1)
		{
			if(!queryString.contains("&"+actionSubmit))
			{
				return true;
			}
		}	
	...
	private boolean validateMethod(HttpServletRequest request)
	{
		...
		if(null != queryString)
		{
			if(checkActionSubmitInGet(queryString, ACTION_CODE))
			{
				return true;
			}
			
			if(checkActionSubmitInGet(queryString, SUBMIT_FORM))
			{
				return true;
			}
		}
		...
		if(isArjspmorph || isDynValue || isPopUp)
			return true;

		if(request.getRequestURI().contains("index.jsp")||request.getRequestURI().contains("fincore.jsp")||request.getRequestURI().contains("dummy.jsp")||request.getRequestURI().contains("coreapplet.jsp")||request.getRequestURI().contains("global.jsp")||request.getRequestURI().contains("corealert.jsp")||request.getRequestURI().contains("generateJnlp.jsp")||request.getRequestURI().contains("loadApplet.jsp"))
			return true;
