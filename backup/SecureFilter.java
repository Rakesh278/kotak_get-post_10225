...
public class SecureFilter implements Filter, SecurityConstants
{
	...
	private boolean validateMethod(HttpServletRequest request)
	{
		...
		boolean islmlServlet  = reqUri.contains("/lmlServlet");
		boolean isActionCode  = reqUri.contains("?actionCode=");
		...
		if(isArjspmorph || islmlServlet || isDynValue || isPopUp || isActionCode )
			return true;
		
		if(request.getRequestURI().contains("index.jsp")||request.getRequestURI().contains("fincore.jsp")||request.getRequestURI().contains("dummy.jsp")||request.getRequestURI().contains("coreapplet.jsp")||request.getRequestURI().contains("corealert.jsp")||request.getRequestURI().contains("generateJnlp.jsp"))
			return true;
	
		if("GET".equals(request.getMethod()))
		{
			dispName="HTTP method";
			dispStr="Invalid";
			return false;
		}
